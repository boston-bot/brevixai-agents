...
def build_duplicate_payment_review_workflow(findings: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    duplicate_findings = [
        ...
        if isinstance(finding, dict) and _is_duplicate_payment_finding(finding)
    ]
    ...
    transaction_ids = _unique_preserving_order(
        transaction_id
        for finding in duplicate_findings
        for transaction_id in _finding_transaction_ids(finding)
        if transaction_id
    )
    ...
    highest_severity = _max_priority(
        *(_normalize_priority(str(finding.get("severity") or "info")) for finding in duplicate_findings)
    )
